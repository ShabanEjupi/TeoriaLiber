// Image Integration Auto-Fix Script
// This script ensures all images are properly integrated before download

(function() {
    'use strict';
    
    console.log('🔧 Loading Image Auto-Fix Script...');

    // Function to ensure images are integrated in all chapters
    function ensureAllImagesIntegrated() {
        ...
        for (let i = 1; i <= totalChapters; i++) {
            if (window.chapters[i] && window.chapters[i].content) {
                ...
                if (!content.includes('chapter-image-container') && !content.includes('<img src="imazhet')) {
                    // Try to add image
                    const imageHTML = createImageHTMLForChapter(i);
                    if (imageHTML) {
        ...
        console.log(`🎯 Image auto-fix complete! Fixed ${fixedCount} chapters.`);
        return fixedCount > 0;
    }

    // Create image HTML for a specific chapter
    function createImageHTMLForChapter(chapterNumber) {
        const imageMap = getImagePathForChapter(chapterNumber);
    ...
    // Get image path for chapter
    function getImagePathForChapter(chapterNumber) {
        ...
        const imageMap = {
            ...
            // For chapters 83-111, we'll use rotating images from the available ones
        ...
        if (imageMap[chapterNumber]) {
            return imageMap[chapterNumber];
        }

        // Otherwise use a rotating selection based on chapter theme
        const rotatingImages = [
            'imazhet-kap-51-111/Create comprehensive success scene Confident Albanian businessman with BMW 530d, family support in background, community respect, Islamic calligraphy suggesting spiritual grounding, modern business success symbols, elegant.jpg',
            'imazhet-kap-51-111/Create crisis management scene Albanian business leader in crisis command center, multiple screens showing business continuity plans, team working from home via technology, safe business operations during challenge, golden.jpg',
            'imazhet-kap-51-111/Create data analytics scene Albanian business analyst in high-tech command center with multiple screens showing business dashboards, data visualizations, predictive models, AI insights, luxury office with Islamic geometric.jpg',
            'imazhet-kap-51-111/Create international expansion scene Albanian entrepreneur opening business in multiple countries, world map with business locations marked, different cultural business meetings, luxury office with international flags, gol.jpg',
            'imazhet-kap-51-111/Create networking event scene Kosovar professionals connecting at luxury venue, handshakes and business cards exchange, cultural diversity, Islamic and Kosovar symbols, golden connection networks. Style social, profes.jpg'
        ];

        return rotatingImages[chapterNumber % rotatingImages.length];
    }

    // Auto-fix when page loads
    function autoFix() {
        if (typeof window.chapters !== 'undefined' && window.chapters) {
            ensureAllImagesIntegrated();
        } else {
    ...
    window.createImageHTMLForChapter = createImageHTMLForChapter;
