...
// Maps all 48 available images to chapters for the Game Theory book

// Complete image mapping - all images from the imazhet folder
...
// Chapter to image mapping - each chapter gets 1-2 relevant images
...
function getChapterImages(chapterNumber) {
    const imageKeys = enhancedChapterImageMap[chapterNumber] || [];
    return imageKeys.map(key => ({
        key: key,
        path: completeImageMap[key],
        alt: `Ilustrim për kapitullin ${chapterNumber}`
    })).filter(img => img.path);
}

// Function to generate image gallery HTML for a chapter
function generateChapterImageGallery(chapterNumber) {
    ...
    if (images.length === 1) {
        ...
        const img = images[0];
        return `
            <div class="chapter-image-gallery single-image">
        ...
        return `
            <div class="chapter-image-gallery multi-images">
                <div class="gallery-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; margin: 20px 0;">
                    ${imageHTML}
                </div>
            </div>
        `;
...
// Function to get all used images
function getUsedImages() {
    const usedKeys = new Set();
    Object.values(enhancedChapterImageMap).forEach(imageKeys => {
        imageKeys.forEach(key => usedKeys.add(key));
    });
    return Array.from(usedKeys);
}

// Function to get unused images
function getUnusedImages() {
    const usedKeys = new Set(getUsedImages());
    return Object.keys(completeImageMap).filter(key => !usedKeys.has(key));
}

// Statistics function
function getImageStatistics() {
    const totalImages = Object.keys(completeImageMap).length;
    const usedImages = getUsedImages().length;
    ...
    return {
        ...
        chaptersWithImages,
        utilizationRate: Math.round((usedImages / totalImages) * 100)
    };
}

console.log('📸 Enhanced Image Integration System Loaded');
console.log('📊 Statistics:', getImageStatistics());
