// Fixed Complete Image Integration Script for All Chapters 1-111
// This script properly maps all images from both folders and ensures they display correctly

(function() {
    'use strict';
    
    console.log('🔧 Loading Fixed Image Integration Script...');

    // Complete and correct image mapping for all chapters 1-111
    const completeImageMap = {
        // Chapters 1-50 from imazhet folder (correct mapping based on actual files)
    ...
    // Function to create image HTML with improved error handling
    function createImageHTML(chapterNumber) {
        const imagePath = completeImageMap[chapterNumber];
        
        if (!imagePath) {
            console.warn(`⚠️ No image mapped for chapter ${chapterNumber}`);
            return '';
        }
            ...
            <div class="chapter-image-container" style="margin: 30px 0; text-align: center; clear: both;">
                <div class="single-chapter-image">
                    <img src="${imagePath}" 
                         ...
                         onload="console.log('✅ Image loaded successfully for chapter ${chapterNumber}: ${imagePath}')"
                         onerror="console.warn('⚠️ Failed to load image for chapter ${chapterNumber}: ${imagePath}'); this.style.display='none';">
                </div>
            </div>
        `;
    }
    ...
    function integrateAllChapterImages() {
                ...
                if (!content.includes('chapter-image-container') && !content.includes('<img src="imazhet')) {
                    ...
                    if (imageHTML) {
                        ...
                        if (titleMatch) {
                            ...
                            window.chapters[i].content = content.slice(0, insertPoint) + 
                                                        '\n\n' + imageHTML + '\n\n' + 
                                                        content.slice(insertPoint);
                            fixedCount++;
                            console.log(`✅ Fixed image for chapter ${i}`);
                        }
                    }
                } else {
                    // Check if existing image needs to be updated with correct path
                    const currentImageMatch = content.match(/src="([^"]+)"/);
                    if (currentImageMatch && completeImageMap[i]) {
                        const currentPath = currentImageMatch[1];
                        const correctPath = completeImageMap[i];
                        
                        if (currentPath !== correctPath) {
                            window.chapters[i].content = content.replace(currentPath, correctPath);
                            fixedCount++;
                            console.log(`✅ Updated image path for chapter ${i}`);
                        }
                    }
                }
            }
        ...
        console.log(`🎯 Fixed Image Integration complete! Fixed ${fixedCount} chapters.`);
        return fixedCount > 0;
    }

    // Function to verify image integration
    function verifyImageIntegration() {
        console.log('🔍 Verifying image integration for all chapters...');
    ...
    // Auto-fix function that runs on page load
    function autoFix() {
        if (typeof window.chapters !== 'undefined' && window.chapters) {
            console.log('🔧 Auto-fixing image integration...');
    ...
    window.ensureAllImagesIntegrated = integrateAllChapterImages;
    window.createImageHTMLForChapter = createImageHTML;

    // Auto-execute after a short delay
    setTimeout(autoFix, 1000);

    console.log('✅ Fixed Image Integration Script loaded successfully');
